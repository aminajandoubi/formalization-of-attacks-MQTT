...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, OBS|->FALSE, INJ1|->FALSE, INJ2|->FALSE, pk|->1, pkt|->{}]]
...
DISCONNECT(self) == 
                    ...
                    /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                             /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                                \/ /\ (packet.type = "SUBSCRIBE")
                                      /\ LET __attaque_data == [_attaque_data EXCEPT ![self].pkt = packet.topic] IN
                                         ...
                                         \/ /\ ~((___attaque_data[self].OBS = TRUE))
                                                  /\ _attaque_data' = ___attaque_data
                                                  /\ _pc' = [_pc EXCEPT ![self] = "DISCONNECT"]
                                                  /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                                \/ /\ ~((packet.type = "SUBSCRIBE"))
...
CONNECT(self) == 
                 /\ _pc[self] = "CONNECT"
                 /\ cp = any
                 /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 50]) IN
...
SUBSCRIBE(self) == 
                   /\ _pc[self] = "SUBSCRIBE"
                   /\ cp = any
                   /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "SUBSCRIBE", topic |-> _attaque_data[self].pkt, sender |-> 50]) IN
                      ...
                      /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
_attaque(self) == 
                      DISCONNECT(self) \/ CONNECT(self) \/ SUBSCRIBE(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
...
Temp0 == (~ \A I \in attaque : (((_attaque_data[I].OBS = TRUE) /\ (<> (_attaque_data[I].INJ1 = TRUE))) => (<> (_attaque_data[I].INJ2 = TRUE))))
