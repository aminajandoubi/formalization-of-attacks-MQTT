...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, OBS|->FALSE, INJ|->FALSE, pk|->1]]
...
CONNECT(self) == 
                 ...
                 /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                          /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet.type = "CONNECT")
                                   /\ LET __attaque_data == [_attaque_data EXCEPT ![self].pk = packet.sender] IN
                                      LET _network == send(__attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> packet.sender]) IN
                                      LET __network == [_network EXCEPT ![__attaque_data[self].BrokerID] = Tail(_network[__attaque_data[self].BrokerID])] IN
                                      LET ___attaque_data == [__attaque_data EXCEPT ![self].OBS = TRUE] IN
                                      \/ /\ (___attaque_data[self].OBS = TRUE)
...
CONNACK(self) == 
                 /\ _pc[self] = "CONNACK"
                 /\ cp = any
                 /\ \/ /\ (Len(network[_attaque_data[self].pk]) > 0)
                          /\ LET packet1 == Head(network[_attaque_data[self].pk]) IN
                             \/ /\ (packet1.type = "CONNACK")
                                   /\ LET __attaque_data == [_attaque_data EXCEPT ![self].INJ = TRUE] IN
                                      LET _network == [network EXCEPT ![__attaque_data[self].pk] = Tail(network[__attaque_data[self].pk])] IN
                             ...
                             \/ /\ ~((packet1.type = "CONNACK"))
                                   /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                      ...
                                      \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      CONNECT(self) \/ CONNACK(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
