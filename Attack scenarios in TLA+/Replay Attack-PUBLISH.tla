...
VARIABLES depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack, _pc

vars == << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack, _pc >>
...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, OBS|->FALSE, INJ|->FALSE, pk|->1, pk1|->1]]
...
Lbl_1(self) == 
               ...
               /\ \/ /\ (({Head(_stack[self]).pkt.sender} \cap activeClients) # {})
                        /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBCOMP", sender |-> self]) IN
                           ...
                           /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                  \/ /\ ~((({Head(_stack[self]).pkt.sender} \cap activeClients) # {}))
                           /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                           /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_2(self) == 
               /\ _pc[self] = "Lbl_2"
               /\ (cp = any \/ cp = self)
               /\ \/ /\ (({Head(_stack[self]).pkt.sender} \cap activeClients) # {})
                                  ...
                                  \/ /\ (pkt.type = "PUBCOMP")
                                        /\ LET _newhead == Head(_stack[self]) IN
                                           ...
                                           \/ /\ (Head(___stack[self]).PUBSUCC = TRUE)
                                                    ...
                                                    /\ _stack' = ___stack
                                                    /\ network' = _network
                                                    /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                                           \/ /\ ~((Head(___stack[self]).PUBSUCC = TRUE))
                                                    /\ _stack' = ___stack
                                                    /\ network' = _network
            ...
            /\ \/ /\ (Head(_stack[self]).pkt.QoS = 0)
               ...
               \/ /\ (Head(_stack[self]).pkt.QoS = 2)
                     /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBREC", sender |-> self]) IN
                        ...
                        /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                        /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_4(self) == 
               /\ _pc[self] = "Lbl_4"
               /\ (cp = any \/ cp = self)
               /\ IF TopicPool[Head(_stack[self]).pkt.topic] # {} /\ Head(_stack[self]).idS_sub = {}
                        THEN /\ LET newHead1 == Head(_stack[self]) IN
                                ...
                                /\ _pc' = [_pc EXCEPT ![self] = "Lbl_4"]
                                /\ _stack' = ___stack
                                /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                        ELSE
                         ...
                         /\ LET packet == Head(network[self]) IN
                            \/ /\ (packet.type = "CONNECT")
                                  /\ LET _activeClients == (activeClients \cup {packet.sender}) IN
                                     ...
                                     /\ UNCHANGED << depth, cp, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                            \/ /\ (packet.type = "PUBLISH")
                                  /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                            ...
                            \/ /\ (packet.type = "SUBSCRIBE")
                                  /\ \/ /\ (({packet.sender} \cap activeClients) # {})
                                           /\ LET _TopicPool == [TopicPool EXCEPT ![packet.topic] = (TopicPool[packet.topic] \cup {packet.sender})] IN
                                              ...
                                              /\ network' = __network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, activeClients, Topics, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                                     \/ /\ ~((({packet.sender} \cap activeClients) # {}))
                                           /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                                              /\ network' = _network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                              ...
                              \/ /\ (packet.type = "CONNACK")
                                    /\ LET __Publisher_data == [_Publisher_data EXCEPT ![self].CONNSUCC = TRUE] IN
                                       ...
                                       \/ /\ (__Publisher_data[self].CONNSUCC = TRUE)
                                                ...
                                                /\ network' = _network
                                                /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Subscriber_data, _attaque_data, _stack >>
                                       \/ /\ ~((__Publisher_data[self].CONNSUCC = TRUE))
                                                /\ _Publisher_data' = __Publisher_data
                                                /\ network' = _network
                                  ...
                                  \/ /\ (packet.type = "CONNACK")
                                        /\ LET __Subscriber_data == [_Subscriber_data EXCEPT ![self].CONNSUCC = TRUE] IN
                                           ...
                                           \/ /\ (__Subscriber_data[self].CONNSUCC = TRUE)
                                                    ...
                                                    /\ network' = _network
                                                    /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _attaque_data, _stack >>
                                           \/ /\ ~((__Subscriber_data[self].CONNSUCC = TRUE))
                                                    /\ _Subscriber_data' = __Subscriber_data
                                                    /\ network' = _network
...
PUBLISH(self) == 
                 ...
                 /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                          /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet.type = "PUBLISH")
                                   /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBLISH", sender |-> packet.sender, topic |-> packet.topic, QoS |-> packet.QoS]) IN
                                      LET __attaque_data == [_attaque_data EXCEPT ![self].pk = packet.QoS] IN
                                      LET ___attaque_data == [__attaque_data EXCEPT ![self].pk1 = packet.sender] IN
                                      LET ____attaque_data == [___attaque_data EXCEPT ![self].OBS = TRUE] IN
                                      \/ /\ (____attaque_data[self].OBS = TRUE)
                                               /\ _pc' = [_pc EXCEPT ![self] = "ResultatQoS0"]
                                               /\ network' = _network
                                               /\ _attaque_data' = ____attaque_data
                             ...
                             \/ /\ ~((packet.type = "PUBLISH"))
                                   /\ \/ /\ (_attaque_data[self].OBS = TRUE)
                                      ...
                                      \/ /\ ~((_attaque_data[self].OBS = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
ResultatQoS0(self) == 
                      /\ _pc[self] = "ResultatQoS0"
                      /\ cp = any
                      /\ \/ /\ (_attaque_data[self].pk = 0)
                               /\ LET __attaque_data == [_attaque_data EXCEPT ![self].INJ = TRUE] IN
                                  /\ _attaque_data' = __attaque_data
                                  /\ _pc' = [_pc EXCEPT ![self] = "ResultatQoS1"]
                                  /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                         \/ /\ ~((_attaque_data[self].pk = 0))
                                  /\ _pc' = [_pc EXCEPT ![self] = "ResultatQoS1"]
                                  /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
ResultatQoS1(self) == 
                      /\ _pc[self] = "ResultatQoS1"
                      /\ cp = any
                      /\ \/ /\ (_attaque_data[self].pk = 1)
                                  /\ _pc' = [_pc EXCEPT ![self] = "Lbl_7"]
                                  /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                         \/ /\ ~((_attaque_data[self].pk = 1))
                                  /\ _pc' = [_pc EXCEPT ![self] = "ResultatQoS2"]
                                  /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_7(self) == 
               /\ _pc[self] = "Lbl_7"
               /\ cp = any
               /\ \/ /\ (Len(network[_attaque_data[self].pk1]) > 0)
                        /\ LET packet1 == Head(network[_attaque_data[self].pk1]) IN
                           \/ /\ (packet1.type = "PUBACK")
                                 /\ LET __attaque_data == [_attaque_data EXCEPT ![self].INJ = TRUE] IN
                                    LET _network == [network EXCEPT ![__attaque_data[self].pk1] = Tail(network[__attaque_data[self].pk1])] IN
                                    \/ /\ (__attaque_data[self].INJ = TRUE)
                                             /\ _pc' = [_pc EXCEPT ![self] = "ResultatQoS2"]
                                             /\ _attaque_data' = __attaque_data
                                             /\ network' = _network
                                             /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                           ...
                           \/ /\ ~((packet1.type = "PUBACK"))
                                 /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                    ...
                                    \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                             ...
                                             /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
ResultatQoS2(self) == 
                      /\ _pc[self] = "ResultatQoS2"
                      /\ cp = any
                      /\ \/ /\ (_attaque_data[self].pk = 2)
...
Lbl_8(self) == 
               ...
               /\ cp = any
               /\ \/ /\ (Len(network[_attaque_data[self].pk1]) > 0)
                        /\ LET packet2 == Head(network[_attaque_data[self].pk1]) IN
                           \/ /\ (packet2.type = "PUBREC")
                                 /\ LET __attaque_data == [_attaque_data EXCEPT ![self].INJ = TRUE] IN
                                    LET _network == [network EXCEPT ![__attaque_data[self].pk1] = Tail(network[__attaque_data[self].pk1])] IN
                                    \/ /\ (__attaque_data[self].INJ = TRUE)
                           ...
                           \/ /\ ~((packet2.type = "PUBREC"))
                                 /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                    ...
                                    \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                             ...
                                             /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      PUBLISH(self) \/ ResultatQoS0(self) \/ ResultatQoS1(self) \/ Lbl_7(self) \/ ResultatQoS2(self) \/ Lbl_8(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
...
Temp0 == (~ \A I \in attaque : ((_attaque_data[I].OBS = TRUE) => (<> (_attaque_data[I].INJ = TRUE))))
