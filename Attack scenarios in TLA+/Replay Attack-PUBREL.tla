...
PUBREL(self) == 
                ...
                /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                         /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                            \/ /\ (packet.type = "PUBREL")
                                  /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBREL", sender |-> packet.sender]) IN
                             ...
                             \/ /\ ~((packet1.type = "PUBCOMP"))
                                   /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                      ...
                                      \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      PUBREL(self) \/ PUBCOMP(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
