...
SUBSCRIBE(self) == 
                   ...
                   /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                            /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                               \/ /\ (packet.type = "SUBSCRIBE")
                                     /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "SUBSCRIBE", topic |-> packet.topic, sender |-> packet.sender]) IN
                                        LET __attaque_data == [_attaque_data EXCEPT ![self].OBS = TRUE] IN
                                        LET ___attaque_data == [__attaque_data EXCEPT ![self].pk = packet.sender] IN
                                        \/ /\ (___attaque_data[self].OBS = TRUE)
                                                 /\ _pc' = [_pc EXCEPT ![self] = "SUBACK"]
                                                 /\ network' = _network
                            ...
                            \/ /\ ~((packet1.type = "SUBACK"))
                                  /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                     ...
                                     \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                              ...
                                              /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      SUBSCRIBE(self) \/ SUBACK(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
